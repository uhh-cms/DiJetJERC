...
setup_dijet() {
    ...
    #   - "DIJET_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "DIJET_" variables is documented below.
    #
    ...
    export PATH="${DIJET_BASE}/bin:${PATH}"
    export PYTHONPATH="${DIJET_BASE}:${DIJET_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${DIJET_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${DIJET_BASE}/modules" ); do
            cf_init_submodule "${DIJET_BASE}" "modules/${m}"
        done
    fi
...
main() {
    ...
    if setup_dijet "$@"; then
        cf_color green "DiJet analysis successfully setup"
        return "0"
    else
        local code="$?"
