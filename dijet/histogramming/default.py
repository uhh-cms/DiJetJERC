...
"""
Special histogram producers for JER.
"""
...
from columnflow.hist_util import translate_hist_intcat_to_strcat
from columnflow.util import maybe_import
from columnflow.types import TYPE_CHECKING, Any

from dijet.hist_util import create_hist_from_variables, fill_hist
...
@hist_producer()
def weighted_mean(self: HistProducer, events: ak.Array, **kwargs) -> ak.Array:
    """
    Custom histogram producer for dijet analysis. Keeps track of x axis

    implements all hooks necessary to ensure columnflow's default behavior:
    ...
    """
    # DEBUG: raw counts -> keep commented out for reference
...
@weighted_mean.init
def weighted_mean_init(self: HistProducer) -> None:
    weight_columns = set()
...
@weighted_mean.create_hist
def weighted_mean_create_hist(
    self: HistProducer,
    ...
    **kwargs,
    ...
    """
    Define the histogram structure for the weighted_mean histogram producer.
    """
...
@weighted_mean.fill_hist
def weighted_mean_fill_hist(self: HistProducer, h: hist.Hist, data: dict[str, Any], task: law.Task) -> None:
    """
...
@weighted_mean.post_process_hist
def weighted_mean_post_process_hist(self: HistProducer, h: hist.Hist, task: law.Task) -> hist.Hist:
    """
