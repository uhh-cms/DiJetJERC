...
def extract_width(
    ...
    * ``width``: the width of the response distributions
    ...
    outputs["mpf_qsub"] = {
        "width": hist_sub_in_quadrature(
            outputs["mpf_y"]["width"],
            outputs["mpf_x"]["width"],
        ),
        # use number of events from y response (TODO: improve?)
        "nevt": inputs["mpf_y"]["nevt"].copy(),
    }
...
def extrapolate_width(
    ...
    * ``extrapolation``: the extrapolated width of the response distribution, calculated via a linear
    ...
    outputs["mpf_qsub"]["extrapolation"]["intercepts_qsub_after_extrapolation"] = hist_sub_in_quadrature(
