...
"""
Base implementations of JER postprocessing steps.
"""
...
#
# generic functions for post-processing steps
#
...
def trim_tails_impl(
    ...
    * ``cut_edges``: a dict of histograms ``low`` and ``up`` containing the lower and upper bound
    ...
    for response_key, response_cfg in self.responses.items():
        ...
        ind_lo = np.apply_along_axis(
            np.searchsorted,
            axis_indices[response_var_key],
            fraction,
            quantile_lo,
            side="left",
        )
        ind_up = np.apply_along_axis(
            np.searchsorted,
            axis_indices[response_var_key],
            fraction,
            quantile_up,
